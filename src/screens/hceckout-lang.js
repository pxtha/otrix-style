    ...
    return (
        ...
        <OtrixContainer customStyles={{ backgroundColor: Colors().light_white }}>
                    ...
                    <View style={[styles.indicator1, { marginRight: wp('4%') }]}>
                        <View style={{ position: 'relative' }}>
                            <View style={[styles.ract, { borderColor: step == 1 ? Colors().themeColor : 'transparent' }]}>
                                ...
                                <Text style={[styles.indicatorText, { color: step == 1 ? Colors().themeColor : Colors().secondry_text_color }]}>{strings.chekout.address}</Text>
                            </View>
                    ...
                    <View style={styles.indicator1}>
                        <View style={{ borderColor: step == 2 ? Colors().themeColor : 'transparent' }}>
                            <View style={[styles.ract, { borderColor: step == 2 ? Colors().themeColor : 'transparent' }]}>
                                <Text style={[styles.indicatorText, { color: step == 2 ? Colors().themeColor : Colors().secondry_text_color }]}>{strings.chekout.payment}</Text>
                            </View>
                            <View style={[styles.tri]}>
            ...
            {!loading && step == 1 && <>
                ...
                <Text style={styles.deliveryTitle}>{strings.chekout.delivery_address}</Text>
                <OtrixDivider size={"md"} />
                {
                    addresses.length > 0 &&
                                ...
                                addresses.map((item, index) =>
                                    ...
                                    >
                                        ...
                                        {selctedAddress == item.id &&
                                            <Text style={styles.deliveryAddressTxt}>{strings.chekout.delivery_address} <Icon name="md-checkmark-circle-sharp" color={Colors().themeColor} size={wp('4%')} style={{ textAlignVertical: 'center' }} /></Text>
                                        }
                                        <TouchableOpacity style={[styles.editView, { bottom: selctedAddress == item.id ? hp('12%') : hp('10%') }]} onPress={() => editAddress(item)}>
...

            {
                !loading && addresses.length == 0 && <TouchableOpacity style={styles.noaddress} onPress={() => setState({ ...state, showAdd: true })}>
                    <Text style={[styles.addressTxt]} > <MatIcon name="plus" color={Colors().link_color} size={wp('5%')} />{strings.chekout.add_new_address} </Text>
                </TouchableOpacity>
            }
            ...
            {!loading && step == 1 && <>
                ...
                {/* Address Content start from here */}
                <OtrixDivider size={"md"} />
                <Text style={styles.deliveryTitle}>{strings.chekout.shipping_method}</Text>
                <OtrixDivider size={"md"} />
                {
            ...
            {/******** PAYMENT SECTION *************/}
            {
                !loading && step == 2 && <View style={{ marginHorizontal: wp('4%') }}>
                    ...
                    <OtrixDivider size={"md"} />
                    <Text style={styles.paymentMethodTitle}>{strings.chekout.payment_methods}</Text>
                    <OtrixDivider size={"sm"} />
                    {
            ...
            {
                !loading && step == 2 && <View style={{ marginHorizontal: wp('4%') }}>
                    <Text style={[styles.summayTitle, { left: wp('1.5%') }]}>{strings.chekout.order_comments}</Text>
                    <OtrixDivider size={"sm"} />
            ...
            <OtrixDivider size={"md"} />
            {
                !loading && step == 2 && <OtrixContent>
                    <OtrixDivider size={"sm"} />
                    <Text style={styles.summayTitle}>{strings.chekout.order_summary}</Text>
                    <OtrixDivider size={"sm"} />
...

            {
                step == 1 && <View style={[styles.totalView, { justifyContent: 'flex-end', bottom: hp('1.5%') }]}>
                    ...
                    >
                        <Text style={GlobalStyles.buttonText}>{strings.chekout.proceed}</Text>
                    </Button>
                </View>
            }
            {
                ...
                <View style={styles.checkoutView}>
                    ...
                    <View style={styles.totalView}>
                        ...
                        <Text style={[styles.rightTxt, { color: Colors().link_color, fontSize: wp('5.5%') }]}>{grandTotal}</Text>
                        <Button
                            size="md"
                            variant="solid"
                            bg={'#0ab97a'}
                            ...
                            onPress={() => payOnline()}
                        >
                            <Text style={[GlobalStyles.buttonText, { fontSize: wp('4.8%') }]}>{strings.chekout.place_order}</Text>
                        </Button>
                    </View>
                </View>
...
const styles = StyleSheet.create({
    ...
    tri: {
        position: 'absolute',
        top: I18nManager.isRTL == true ? hp('0.95%') : hp('0.6%'),
        right: I18nManager.isRTL == true ? -wp('2.9%') : wp('2.6%'),
        // right: wp('2.6%')
    },
    arrow: {
        borderTopWidth: 1,
        ...
        backgroundColor: Colors().white,
        borderColor: '#007299',
        width: I18nManager.isRTL == true ? 25 : 20,
        height: I18nManager.isRTL == true ? 25 : 21,
        transform: I18nManager.isRTL == true ? [{ rotate: '-54deg' }] : [{ rotate: '45deg' }]
    },
    indicatorText: {
        fontFamily: Fonts.Font_Semibold,
        fontSize: wp('3.8%'),

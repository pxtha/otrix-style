            ...
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', marginHorizontal: wp('1%') }}>
                {
                    productDetail && productDetail.map((item, index) =>
                        <TouchableOpacity style={[styles.box, styles.sizeBox, { borderColor: selectedSize == item.id ? Colors().themeColor : 'rgb(225, 225, 225)' }]} key={index} onPress={() => setState({ ...state, selectedSize: item.id })}>
                            <Text style={[styles.sizeTxt, { color: selectedSize == item.id ? Colors().themeColor : Colors().secondry_text_color }]}>{item.name}</Text>
                        </TouchableOpacity>
                    )
                }
                {
                    !productDetail && <Text style={[styles.sizeTxt]}>{"Free size"}</Text>
                }
